...
> 状态:已实施(2026-07-12)。两个特性相互独立;阶段二依赖 `FramedPhoto` 抽取。
...
- 三个键都进 `meta.branding`(timeline schema v 保持不变,新增可选字段,老
...
```
tsuzuri still <photo|folder> [-o <out.png|dir>] [--exif] [--scale N]
```

- `<photo>`:单张图,默认输出到 `<照片所在文件夹>/output/stills/<名>.png`。
...
灯、曝光补偿、色彩空间——都是噪音。全部字段缺失时警告一行并回退到无 EXIF
布局。
...
6. **CLI 解析**:`cli/options.mjs` 增加 `still` 子命令(photo|folder、
   `-o`、`--exif`、`--scale`,scale 限 1–4 整数);`USAGE` 更新;
...
7. **渲染器:抽 `FramedPhoto` + 新 `Still` composition**
   ...
   - 新建 `Still.tsx`:props `{src, background, photoScale, exif?: {camera?, lens?, params?, datetime?}}`,
...
- 新路径 smoke:临时素材夹分别验证 ①`outro_text` 中文文案 ②自定义
  `signature.svg` ③`intro=false` ④`still` 单张/文件夹/`--exif`/`--scale`。

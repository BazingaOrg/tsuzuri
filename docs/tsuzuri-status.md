...
交互入口重复显示全局图例：回车执行题目所示的安全默认动作，`0` 在可用时返回，`q` 从任意
交互问题正常退出整个程序，Ctrl+C 以 130 中断。确认题使用具体动作键而非抽象 y/n；危险操作
默认取消且必须显式输入删除/替换键。列表与输入题会原地提示无效值，歌词关键词题明确显示
“回车搜索 · 0 返回”。
...
最近一次本地验证（2026-07-15）：analyzer 80 项、CLI 101 项测试通过，renderer 类型检查通过；

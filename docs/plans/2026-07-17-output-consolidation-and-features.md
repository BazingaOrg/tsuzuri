...
- 选"播完整首歌"后的 `term.start('按裁剪选择重新规划照片时间线')` 去掉,
...
1. [ ] renderer:scale 基准改造 + 纵排 EXIF 布局 + 三种画布的构图核对
       → verify: still 三种画布各导一张目测;renderer 类型检查
2. [ ] CLI:`--portrait` / `--square` 解析、互斥校验、变体后缀、菜单加问
       → verify: options/menu 单测
3. [ ] 端到端各渲一条,ffprobe 校验分辨率,手机上目测小红书式竖屏观看效果
4. [ ] docs 同步

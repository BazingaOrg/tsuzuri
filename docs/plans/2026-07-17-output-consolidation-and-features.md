...
### 阶段 E follow-up（2026-07-17）

- 本地 Yellow（29.301 秒、12 张照片）复测发现：uniform 保留 12 张，dynamic 因前段候选吸附偏晚只生成 9 个切换点，随后由 plan 层提示丢弃末尾照片。这与“dynamic 只重分布、总照片数不变”的设计不符。
- 根因是 `allocate_switch_points` 只对当前点施加 `min_gap` 下界，没有为剩余切换点及最后一张照片预留容量；动态理想网格偏晚时，前点合法却会挤占尾部必需空间。
- 修复为每个第 i 个切换点引入 `duration - (n_photos - 1 - i) * min_gap` 的硬容量上界；候选和回退都限制在 `[lower, capacity_cap]`。`not_after` 仍是软上界，和 `min_gap` 冲突时后者优先。补充 allocator、energy pacing 和 plan 层回归；本条只记录修复与定向测试，未在此虚报 Yellow 端到端复测结果。
- Review P1：初版容量测试只覆盖了无候选时的回退网格，未实际复现候选吸附导致的尾部容量耗尽。现改为 Yellow 前 29.301 秒提取并脱敏的 beats/downbeats/energy 紧凑 fixture，直接断言候选吸附、11 个切换点和尾部间隔；plan 层也使用同一非空 downbeats 形状。该修正不代表已完成真实 demo 的修后端到端复测。
- 最终真实 demo QA：Yellow 29.301 秒、12 张照片的 uniform/dynamic 均保留 12 photo / 11 switches，无“丢弃末尾”；actual `min_gap >= 2s`、tail `>= 2s`。两条 draft 视频均为 1280×720、29.354667 秒；energy 为 374 项、范围 `[0, 1]`，高能窗口的切换占比由 uniform 的 3/11 增至 dynamic 的 4/11。原 demo 的 22 项 SHA-256 均保持一致。
